...
const EditCollateral = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      if (!id) {
        setFormError('Invalid collateral id');
      ...
      try {
        ...
        const [borrowerRes, loansRes, collateral] = await Promise.all([
        ...
        setFormData({
          borrowerId: collateral.borrowerId?._id || '',
          loanId: collateral.loanId?._id || '',
          productName: collateral.productName || '',
          type: collateral.type || '',
          serialNumber: collateral.serialNumber || '',
          value: String(collateral.value || 0),
          status: collateral.status || 'Deposited'
        });
      } catch (err: any) {
        setFormError(err?.response?.data?.error || 'Failed to load collateral details');
      } finally {
        setPageLoading(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateCollateral(id, {
        ...
        status: formData.status as CollateralStatus
      });
      navigate('/collateral/view');
    } catch (err: any) {
      setFormError(err?.response?.data?.error || 'Failed to update collateral');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Edit Collateral</h1>
      ...
      {pageLoading ? (
        <div className="bg-white dark:bg-gray-800 p-6 rounded border dark:border-gray-700 text-gray-600 dark:text-gray-300">Loading collateral details...</div>
      ) : (
        <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 p-6 rounded border dark:border-gray-700 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input name="productName" value={formData.productName} onChange={handleChange} required className="w-full border dark:border-gray-700 rounded px-3 py-2 dark:bg-gray-900 dark:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500" />
            </div>
            <div>
              <label className="block text-sm mb-1 text-gray-700 dark:text-gray-300">Type</label>
              <input name="type" value={formData.type} onChange={handleChange} required className="w-full border dark:border-gray-700 rounded px-3 py-2 dark:bg-gray-900 dark:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input name="serialNumber" value={formData.serialNumber} onChange={handleChange} required className="w-full border dark:border-gray-700 rounded px-3 py-2 dark:bg-gray-900 dark:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500" />
            </div>
            <div>
              <label className="block text-sm mb-1 text-gray-700 dark:text-gray-300">Estimated Value</label>
              <input name="value" type="number" min="1" step="0.01" value={formData.value} onChange={handleChange} required className="w-full border dark:border-gray-700 rounded px-3 py-2 dark:bg-gray-900 dark:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500" />
            </div>
          </div>

          <div>
            <label className="block text-sm mb-1 text-gray-700 dark:text-gray-300">Status</label>
            <select name="status" value={formData.status} onChange={handleChange} className="w-full border dark:border-gray-700 rounded px-3 py-2 dark:bg-gray-900 dark:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button disabled={loading} className="px-4 py-2 bg-blue-600 text-white rounded disabled:opacity-50">{loading ? 'Saving...' : 'Save Changes'}</button>
          </div>
        </form>
      )}
    </div>
